...
5. **Router** (`routes/index.tsx`):
   ...
   - Public: `/login`.
   - Protected (envueltas en `<ProtectedRoute>` + `<AppLayout>`): `/`, `/products`, `/products/:id/variants`, `/brands`, `/combos`, `/events`, `/sales`, `/sales/new`, `/sales/:id`.
   - 404 catch-all.
...
7. **`routes/AppLayout.tsx`** — layout principal:
   ...
   - Items del nav: Dashboard, Productos, Marcas, Combos, Eventos, Nueva Venta, Ventas. (Usuarios → fase futura). _(Inventario eliminado: redundante con Productos.)_
...
**Done cuando**:
- [x] Lazy load funciona (verificar Network tab). _Cada page baja en su propio chunk on-demand._
- [x] Bundle prod razonable (<500KB gzipped es buen target). _Initial ≈ 158 KB gz; vendor split en react/router/query/radix/icons/dates/forms/misc._
- [ ] Lighthouse pasa los thresholds. _Pendiente: correr en browser real (DevTools → Lighthouse modo Mobile)._
- [x] Commit: `chore: hardening (lazy load, code splitting, build optimization)`.
...
- [x] Fase 7: hardening _(Lighthouse audit en iPad real / simulador iOS pendiente — manual)_
- [ ] Fase 7: hardening
